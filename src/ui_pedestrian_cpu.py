...
Controlador y GUI para Cruce Peatonal Mid-Block (Motor CPU PyTorch)
...
class SemaforoControllerPedestrian_CPU(CoreSemaforoBase):
    def __init__(self, port=None, video_source=None):
        super().__init__(topology_name="pedestrian", backend_name="CPU", port=port, video_source=video_source)
    ...
    def _init_model(self):
        ...
        for p in possible_paths:
            if os.path.exists(p):
                model_path = p
                break
        print(f"📦 Cargando modelo YOLO en CPU para Cruce Peatonal: {model_path}")
...
class AppPedestrian_CPU:
    def __init__(self, root, video_source=None):
        self.ventana = root
        self.ventana.title("FLUXA - Cruce Peatonal (CPU)")
        self.ventana.geometry("1100x750")
        self.ventana.configure(bg="#0f172a")
        self.ventana.protocol("WM_DELETE_WINDOW", self.on_closing)
        
        self.controller = SemaforoControllerPedestrian_CPU(video_source=video_source)
    ...
    def _crear_ui(self):
        ...
        tk.Label(header, text="FLUXA - CRUCE PEATONAL INTELIGENTE (CPU)", fg="#10b981", bg="#020617", font=("Segoe UI", 18, "bold")).pack(side="left", padx=25, pady=15)
