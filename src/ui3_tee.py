...
Controlador y GUI para Intersección en T de 3 Vías (Motor NPU RKNN)
...
class SemaforoController3V_RKNN(CoreSemaforoRKNN):
    def __init__(self, port=None, video_source=None, npu_core_id=None):
        super().__init__(topology_name="3_way_t", port=port, video_source=video_source, npu_core_id=npu_core_id)
...
class App3V_RKNN:
    def __init__(self, root, video_source=None):
        self.ventana = root
        self.ventana.title("FLUXA - Intersección en T (RKNN NPU)")
        self.ventana.geometry("1100x750")
        self.ventana.configure(bg="#0f172a")
        self.ventana.protocol("WM_DELETE_WINDOW", self.on_closing)
        
        self.controller = SemaforoController3V_RKNN(video_source=video_source)
    ...
    def _crear_ui(self):
        ...
        tk.Label(header, text="FLUXA RKNN - INTERSECCIÓN EN T (ORANGE PI 5)", fg="#f59e0b", bg="#020617", font=("Segoe UI", 18, "bold")).pack(side="left", padx=25, pady=15)
